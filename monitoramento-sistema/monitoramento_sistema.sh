...
function monitorar_disco() {
	...
	df -h | grep -v "snapfuse" | awk '$5+0 > 70 {print $1 " Esta com " $5 " de uso."}' >> $LOG_DIR/monitoramento_disco.txt
	 ...
	 du -sh /home/tai_padrao >> $LOG_DIR/monitoramento_disco.txt
}
...
function monitorar_hardware() {
	 ...
	 echo "Operadores de leitura e escrita:" >> $LOG_DIR/monitoramento_hardware.txt
	iostat | grep -E "Device|^sda|^sdb|^sdc|^sdd" | awk '{print $1, $2, $3, $4}' >> $LOG_DIR/monitoramento_hardware.txt 
}
